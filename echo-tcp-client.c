...
int main(int argc, char **argv)
{
    ...
    msg = strndup(argv[aux], MAX_LENGHT);
    ...
    aux = connect(sockfd,(struct sockaddr*)&server_addr, sizeof(server_addr));
    ASSERT(aux == 0, "Error conectando con el servidor %s\n",strerror(errno));

    // Enviamos mensaje al servidor
    aux = send(sockfd, msg, strlen(msg) + 1, 0);
    ASSERT(aux != -1, "Error enviando mensaje: %s\n", strerror(errno));
    printf("Cadena enviada, esperando respuesta...\n");

    // Recibimos mensaje del servidor
    aux = recv(sockfd, msg, strlen(msg) + 1, 0);
